...
import imagen1 from '../imgs/room1.jpg';
import imagen2 from '../imgs/room2.jpg';
import imagen3 from '../imgs/room3.jpg';
import './bookroom.css'
...
const Bookroom = () => {
  ...
  const handleRoomChange = (e) => {
    ...
    if (!Number.isInteger(Number(roomNumber))) {
      setRoomNumberError('Room number should be a number.');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const roomExists = availableRooms.some(room => room.number === parseInt(selectedRoom, 10));
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/bookings/checkAvailability', {
        method: 'POST',
        headers: {
      ...
      if (!data.available) {
        setBookingError('Room number is already booked for the selected dates.');
        return;
      }
    } catch (error) {
        ...
        <Col md={6}>
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group controlId="category">
              ...
              <Form.Control as="select" onChange={handleCategoryChange} required>
                {/* Options */}
              </Form.Control>
            </Form.Group>
            ...
            <Form.Group controlId="roomNumber">
              ...
              <Form.Control
                ...
                required
              />
              <Form.Control.Feedback type="invalid">{roomNumberError}</Form.Control.Feedback>
            </Form.Group>
